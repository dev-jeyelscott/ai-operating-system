...
test(
    'document upload records attributable redacted lifecycle evidence',
    function (): void {
        config()->set(
...
Approved architecture baseline for lifecycle audit evidence.
MARKDOWN,
            ),
        ...
        expect($event)
            ...
            ->subject_type->toBe(
                AuditSubjectType::DocumentVersion,
            )
...
test(
    'duplicate scan delivery does not append duplicate transition evidence',
    function (): void {
        Queue::fake();
        ...
        expect(
            AuditEvent::query()
                ->where(
                    'event_type',
                    AuditEventType::DocumentScanStarted->value,
                )
                ->count(),
        )->toBe(1);
        ...
        expect(
            AuditEvent::query()
                ->where(
                    'event_type',
                    AuditEventType::DocumentScanCompleted->value,
                )
                ->count(),
        )->toBe(1);
    },
