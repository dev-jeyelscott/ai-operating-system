...
/**
 * Create a complete Layer 2 development execution fixture.
 *
...
function aios096Fixture(int $retryLimit = 2): array
{
    ...
    app(SelectNextTicketAndAcquireLease::class)->handle(
        new TicketSelectionRequest(
            ...
            owner: 'aios-096-worker',
        ),
    );

    $fixture['execution'] = $execution;

    $fixture['lease'] = TicketExecutionLease::query()
...
test(
    'development simulation completes with ordered events and unverified artifacts',
    function (): void {
        ...
        expect($fixture['execution']->refresh()->status)
            ...
            ->and($fixture['lease']->refresh()->release_reason)
            ->toBe(TicketLeaseReleaseReason::Completion)
            ->and(Artifact::query()->count())
            ->toBe(7)
            ->and(Evidence::query()->count())
            ->toBe(7)
            ->and(
                Artifact::query()
                    ->where('execution_provider', 'simulation')
                    ->where('actual_state', 'unverified')
                    ->where('evidence_still_required', true)
                    ->count(),
            )
            ->toBe(7)
            ->and(
                Evidence::query()
                    ->where('classification', 'simulated_output')
                    ->count(),
            )
            ->toBe(7);
        ...
        expect(
            Artifact::query()
                ...
                ->every(
                    static fn (?string $reference): bool => is_string($reference)
                        && str_starts_with(
                            $reference,
                            'simulation://',
                        ),
...
test(
    'completed development job replay creates no second attempt or artifacts',
    function (): void {
        $fixture = aios096Fixture();

        $job = new ProcessDevelopmentExecutionJob(
            $fixture['execution']->id,
            seed: 96,
        );

        $job->handle(
            app(ProcessDevelopmentExecution::class),
        );
...
test(
    'development job owns no competing queue retry loop',
    function (): void {
        $job = new ProcessDevelopmentExecutionJob(
        ...
        expect($job->tries)
            ...
            ->and($job->uniqueId())
            ->toBe(
                'development-execution:01KYPAB5S2ETWGGMB4TFTVWX1E',
            );
    },
);

test(
    'provider execution occurs outside every database transaction',
    function (): void {
        $fixture = aios096Fixture();
        ...
        $provider = new class($delegate) implements DevelopmentExecutionProvider
        {
            ...
            /**
             * Store the underlying deterministic simulation provider.
             */
            public function __construct(
                private SimulationDevelopmentProvider $delegate,
            ) {}
            ...
            /**
             * Determine whether the delegate supports the capability.
             */
            ...
            /**
             * Capture the transaction level before provider execution.
             */
        ...
        expect($provider->transactionLevel)
            ->toBe($testTransactionLevel);
    },
);

test(
    'provider failure schedules domain retry without releasing lease or reaching for qa',
    function (): void {
        $fixture = aios096Fixture();
        ...
        $provider = new class implements DevelopmentExecutionProvider
        {
            ...
            /**
             * Return deterministic metadata for the failing test provider.
             */
            public function metadata(): ExecutionProviderMetadata
            {
            ...
            /**
             * Support the requested capability for this failure scenario.
             */
            public function supports(
                string $capability,
            ): bool {
            ...
            /**
             * Simulate a transient provider execution failure.
             */
            public function execute(
                DevelopmentExecutionRequest $request,
            ): DevelopmentExecutionResult {
        ...
        expect($fixture['execution']->refresh()->status)
            ...
            ->and($fixture['lease']->refresh()->isActive())
            ->toBeTrue();
...
test(
    'wrong pull request target creates no artifact and cannot reach for qa',
    function (): void {
        $fixture = aios096Fixture();
        ...
        $provider = new class($delegate) implements DevelopmentExecutionProvider
        {
            ...
            /**
             * Support the development capability for this policy test.
             */
            public function supports(
                string $capability,
            ): bool {
            ...
            /**
             * Produce an otherwise valid result with an invalid PR target.
             */
            public function execute(
                DevelopmentExecutionRequest $request,
            ): DevelopmentExecutionResult {
                ...
                /*
                 * Clear and recompute the canonical fingerprint because
                 * the test intentionally changes the provider result.
                 */
                $data[
                ...
                $temporary =
                    DevelopmentExecutionResult::fromArray(
                        $data,
                    );
                ...
                $data[
                    ...
                    new DevelopmentResultValidator
                )->fingerprint(
                    $temporary,
                );

                return DevelopmentExecutionResult::fromArray(
                    $data,
                );
            }
        };
...
test(
    'cancellation racing successful provider completion wins before artifacts persist',
    function (): void {
        $fixture = aios096Fixture();
        ...
        {
            /**
             * Store the provider and execution used to simulate cancellation.
             */
            public function __construct(
                private SimulationDevelopmentProvider $delegate,
            ...
            /**
             * Support the requested development capability.
             */
            public function supports(
                string $capability,
            ): bool {
            ...
            /**
             * Request cancellation while provider execution is in flight.
             */
            public function execute(
                DevelopmentExecutionRequest $request,
            ): DevelopmentExecutionResult {
        ...
        expect($fixture['execution']->refresh()->status)
            ...
            ->toBe(TicketStatus::InProgress)
            ->and($fixture['lease']->refresh()->release_reason)
            ->toBe(TicketLeaseReleaseReason::Cancellation);
...
test(
    'DispatchDevelopmentExecution validates lineage and dispatches one unique job',
    function (): void {
        Bus::fake();
        ...
        $event = new StoredDomainEvent(
            eventId: '01KYPAB5S2ETWGGMB4TFTVWX1H',
            eventName: 'ticket.lease_acquired',
            organizationId: $fixture['project']->organization_id,
            projectId: $fixture['project']->id,
            schemaVersion: 1,
            envelope: [
        ...
        Bus::assertDispatched(
            ProcessDevelopmentExecutionJob::class,
            1,
        );
    },
);
...
test(
    'artifact persistence failure rolls back the terminal stage and schedules domain retry',
    function (): void {
        $fixture = aios096Fixture();

        app()->instance(
            DevelopmentArtifactRecorder::class,
            new class extends DevelopmentArtifactRecorder
            {
                /**
                 * Create the failing recorder test double.
                 */
                public function __construct() {}

                /**
                 * Simulate an unavailable artifact persistence layer.
                 *
...
test(
    'DevelopmentValidationFailure persists only failed simulated evidence and schedules retry',
    function (): void {
        $fixture = aios096Fixture();
        ...
        expect($fixture['execution']->refresh()->status)
            ->toBe(ExecutionStatus::RetryScheduled)
            ->and($fixture['execution']->next_attempt_at)
            ->not->toBeNull()
            ->and($fixture['ticket']->refresh()->status)
            ->toBe(TicketStatus::InProgress)
            ->and($fixture['lease']->refresh()->isActive())
        ...
        $this->assertDatabaseMissing(
            'artifacts',
            [
                'artifact_type' => 'synthetic_commit',
            ],
        );
...
test(
    'DevelopmentRetry provider timeout uses domain timing without queue retry or artifacts',
    function (): void {
        $fixture = aios096Fixture();
        ...
        $this->assertDatabaseHas(
            ...
            [
                'execution_id' => $fixture['execution']->id,
                'error_code' => 'development.provider_timeout',
                'retryable' => true,
            ],
        );
...
test(
    'DevelopmentRetry release permits later success while preserving failure history',
    function (): void {
        $fixture = aios096Fixture();
        ...
        $nextAttemptAt =
            $fixture['execution']->next_attempt_at;
        ...
        expect($fixture['execution']->refresh()->status)
            ->toBe(ExecutionStatus::Completed)
            ->and($fixture['execution']->attempt_count)
            ->toBe(2)
            ->and($fixture['ticket']->refresh()->status)
            ->toBe(TicketStatus::ForQa)
            ->and(
        ...
        $this->assertDatabaseHas(
            ...
            [
                'execution_id' => $fixture['execution']->id,
                'attempt_number' => 1,
                'error_code' => 'development.validation_failed',
            ],
        );
...
test(
    'DevelopmentRetry exhaustion fails execution and releases lease exactly once',
    function (): void {
        $fixture = aios096Fixture(
            retryLimit: 0,
        );
...
test(
    'RedispatchDevelopmentRetry dispatches only after durable retry release',
    function (): void {
        Bus::fake();
        ...
        app(
            ...
            $execution->next_attempt_at?->addSecond(),
        );

        $event = new StoredDomainEvent(
            eventId: '01KYPAB5S2ETWGGMB4TFTVWX1J',
