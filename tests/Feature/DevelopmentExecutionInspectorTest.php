...
function aios101InspectorFixture(bool $process = true): array
{
    $fixture = TicketTestFixture::create(
        ticketAttributes: [
            'stable_id' => 'AIOS-101',
            'title' => 'Build execution inspector',
    ...
    $execution = Execution::factory()
        ->for($fixture['project'])
        ->create([
            'project_context_snapshot_id' => $fixture[
                'contextSnapshot'
            ]->id,
...
/**
 * Create an organization member for inspector authorization tests.
 *
...
test(
    'DevelopmentExecutionInspector returns ordered, whitelisted simulated provenance',
    function (): void {
        $fixture = aios101InspectorFixture();

        $inspector = app(
            GetDevelopmentExecutionInspector::class,
        )->handle(
        ...
        expect($inspector['execution']['status'])
            ->toBe('completed')
            ->and($inspector['simulation'])
            ->toBe([
                'simulated' => true,
                'verified' => false,
                'evidenceStillRequired' => true,
            ])
            ->and(array_column($inspector['attempts'], 'number'))
            ->toBe([1])
            ->and($inspector['plan'])
            ->toHaveCount(3)
            ->and($inspector['changedFiles'][0])
            ->toHaveKeys([
                'path',
                'change_type',
                'summary',
            ])
            ->and($inspector['diffSummary'])
            ->toContain(
                'no repository diff was produced',
            )
            ->and(
                $inspector['repository']['branch']['reference'],
            )
            ->toStartWith('simulation://')
            ->and(
                $inspector['repository']['pullRequest']['target_branch'],
            )
            ->toBe('develop')
            ->and($inspector['evidenceGaps'])
            ->not->toBeEmpty()
            ->and($inspector['lease']['recoveryState'])
            ->toBe('released')
            ->and(
                array_column(
                    $inspector['auditTimeline'],
                    'sequence',
                ),
            )
            ->toBe(
                collect($inspector['auditTimeline'])
                    ->pluck('sequence')
                    ->sort()
                    ->values()
                    ->all(),
...
test(
    'DevelopmentExecutionInspector redacts arbitrary artifact metadata and secret-like values',
    function (): void {
        $fixture = aios101InspectorFixture();
        ...
        /*
         * This fixture represents unsafe legacy data written before the
         * persistence observer existed. Production writes must not bypass
         * model events.
         */
        Artifact::withoutEvents(
            static function () use ($attempt): void {
                Artifact::factory()
                    ->forAttempt($attempt)
                    ->create([
        ...
        expect($serialized)
            ->not->toContain('secret-aios-101')
            ->not->toContain('must-not-leak')
            ->not->toContain('private-stack-trace')
            ->and(
                collect($inspector['artifacts'])
                    ->firstWhere(
                        'type',
...
test(
    'DevelopmentExecutionInspector query count stays bounded as artifacts grow',
    function (): void {
        $fixture = aios101InspectorFixture();
        $inspector = app(
            GetDevelopmentExecutionInspector::class,
...
test(
    'DevelopmentExecutionInspector exposes stable attempt order and retry failure state',
    function (): void {
        $fixture = aios101InspectorFixture(
            process: false,
        );
        ...
        expect(
            ...
            ->toBe([1, 2])
            ->and($inspector['retry']['scheduled'])
            ->toBeTrue()
            ->and($inspector['error']['attemptNumber'])
            ->toBe(2)
            ->and($inspector['missingArtifacts'])
            ->toContain(
                'implementation_plan',
...
test(
    'DevelopmentExecutionInspector controller authorizes a member and loads deferred data',
    function (): void {
        $fixture = aios101InspectorFixture();
        ...
        $this->actingAs($user)
            ...
            ->assertInertia(
                fn (Assert $page): Assert => $page
                    ->component(
                        'projects/development/executions/show',
                    )
                    ->where(
                        'project.id',
                        $fixture['project']->id,
                    ...
                    ->loadDeferredProps(
                        fn (Assert $reload): Assert => $reload
                            ->where(
                                'inspector.execution.id',
                                $fixture['execution']->id,
                            )
                            ->where(
                                'inspector.simulation.verified',
                                false,
                            ),
                    ),
            );
...
test(
    'DevelopmentExecutionInspector rejects cross-tenant and cross-project execution reads',
    function (): void {
        $fixture = aios101InspectorFixture();
...
test(
    'DevelopmentExecutionInspector rejects non-development capabilities',
    function (): void {
        $fixture = aios101InspectorFixture(
            process: false,
        );
