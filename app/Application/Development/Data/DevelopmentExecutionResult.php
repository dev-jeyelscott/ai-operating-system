...
final readonly class DevelopmentExecutionResult
{
    ...
    {
        return new self(
            (string) ($data['provider_identifier'] ?? ''), (string) ($data['capability'] ?? ''),
            DevelopmentExecutionOutcome::from((string) ($data['outcome'] ?? '')),
            self::objects($data['stage_results'] ?? null, DevelopmentStageResult::fromArray(...)), self::strings($data['implementation_plan'] ?? null),
            self::objects($data['changed_files'] ?? null, DevelopmentChangedFile::fromArray(...)), (string) ($data['diff_summary'] ?? ''),
            ...
            self::artifact($data['synthetic_pull_request_result'] ?? null), (string) ($data['target_branch'] ?? ''),
            self::strings($data['assumptions'] ?? null), (float) ($data['confidence'] ?? -1), self::strings($data['risks'] ?? null),
            self::strings($data['evidence_gaps'] ?? null),
            DevelopmentSimulationClassification::from((string) ($data['simulation_classification'] ?? '')),
            DevelopmentVerificationClassification::from((string) ($data['verification_classification'] ?? '')),
            DevelopmentFailureClassification::from((string) ($data['retry_classification'] ?? '')),
            (string) ($data['recommended_next_action'] ?? ''), (string) ($data['canonical_result_fingerprint'] ?? ''),
