...
{
    public function approve(Document $document, DocumentVersion $version): void
    {
        $this->review($document, $version, DocumentStatus::Approved);
    }

    public function reject(Document $document, DocumentVersion $version): void
    {
        $this->review($document, $version, DocumentStatus::Rejected);
    }
    ...
    public function supersede(
        ...
        return DB::transaction(function () use ($document, $version): DocumentVersion {
            $document = Document::query()->lockForUpdate()->findOrFail($document->id);
            $source = $this->lockedVersion($document, $version);
            ...
            $source->forceFill(['status' => DocumentStatus::Superseded])->save();
    ...
    private function review(
        ...
        DB::transaction(function () use ($document, $version, $decision): void {
            $source = $this->lockedVersion($document, $version);

            if ($source->status !== DocumentStatus::Parsed) {
                throw new LogicException(
                    'Only a parsed document version can be reviewed.',
                );
            }

            $source->forceFill(['status' => $decision])->save();
        });
    }
